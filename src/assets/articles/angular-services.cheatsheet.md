...
```ts
export const TASKS_TOKEN = new InjectionToken<TasksService>('tasks-service');
```
...
// S vlastným tokenom
providers: [{ provide: TASKS_TOKEN, useClass: TasksService }];
```
...
export const TASK_OPTIONS = new InjectionToken<TaskOptions>('task-options');
...
beforeEach(() => {
  ...
  TestBed.configureTestingModule({
    providers: [TasksService, { provide: LoggingService, useValue: loggerSpy }],
  });
});
```
